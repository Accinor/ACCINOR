...
export default function ProfilePage() {
  ...
  const updateField = useCallback((field: string, value: any) => {
    setProfile_((prev) => prev ? { ...prev, [field]: value } : prev)
  }, [])
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/auth/profile", {
        ...
        body: JSON.stringify({
          full_name: profile_.full_name,
          phone: profile_.phone,
          region: profile_.region,
          city: profile_.city,
          has_project: profile_.has_project,
          project_description: profile_.has_project ? profile_.project_description : null,
          project_stage: profile_.has_project ? profile_.project_stage : null,
        }),
      })
      if (res.ok) {
        setSaved(true)
  ...
  const handleAvatarUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setProfile_((prev) => prev ? { ...prev, avatar_url: data.avatar_url } : prev)
      }
  ...
  const handleAvatarDelete = useCallback(async () => {
    ...
    if (res.ok) {
      setProfile_((prev) => prev ? { ...prev, avatar_url: undefined } : prev)
    }
  }, [])
  ...
  if (authLoading || !profile_) return (
  ...
  const initials = (profile_.full_name || profile_.email)
      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          <div className="relative inline-block">
            {profile_.avatar_url ? (
              <img src={profile_.avatar_url} alt="" className="w-24 h-24 rounded-full object-cover ring-4 ring-[#ffb81b]/20" />
            ) : (
          ...
          <h1 className="text-2xl font-bold text-white mt-4">{profile_.full_name || "User"}</h1>
          <p className="text-gray-400 text-sm">{profile_.email}</p>
          {profile_.role && (
            <span className="inline-block mt-2 px-3 py-1 rounded-full text-xs font-medium bg-[#ffb81b]/15 text-[#ffb81b] border border-[#ffb81b]/20">
              {profile_.role === "admin" ? "Administrator" : profile_.role.charAt(0).toUpperCase() + profile_.role.slice(1)}
            </span>
          )}
        </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 sm:p-8 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Input label="Full Name" value={profile_.full_name} field="full_name" />
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Email</label>
              <input
                type="email"
                value={profile_.email}
                readOnly
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Input label="Phone" value={profile_.phone} field="phone" />
            <Input label="City" value={profile_.city} field="city" />
          </div>
          <Input label="Region" value={profile_.region} field="region" />
          <Input label="Bio" value={profile_.bio} field="bio" type="textarea" />
          <Input label="Title / Position" value={profile_.title} field="title" />
          <Input label="Website" value={profile_.website} field="website" />

          <label className="flex items-center gap-2.5 text-sm text-gray-300 cursor-pointer">
            <input
              type="checkbox"
              checked={profile_.has_project || false}
              onChange={(e) => updateField("has_project", e.target.checked)}
              className="rounded border-white/20 bg-white/5 accent-[#ffb81b]"
            />
            I have a project
          </label>
          {profile_.has_project && (
            <div className="space-y-4 pl-6 border-l border-white/10">
              <Input label="Project Description" value={profile_.project_description} field="project_description" type="textarea" />
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-gray-400 uppercase tracking-wider">Project Stage</label>
                <select
                  value={profile_.project_stage || ""}
