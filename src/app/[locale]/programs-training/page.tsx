...
const formatKeys = ["workshops", "individual", "cohort"] as const
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold mb-4">
            <EditableText page="programs" section="header" field="title" as="span" locale={locale}>
              {t("title")}
            </EditableText>
          </h1>
          <p className="text-xl text-muted-foreground mb-4">
            <EditableText page="programs" section="header" field="subtitle" as="span" locale={locale}>
              {t("subtitle")}
            </EditableText>
          </p>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            <EditableText page="programs" section="header" field="intro" as="span" locale={locale}>
              {t("intro")}
            </EditableText>
          </p>
        </div>
        ...
        <div className="mb-16">
          <h2 className="text-2xl font-semibold mb-6 text-center">{t("format.title")}</h2>
          <div className="grid sm:grid-cols-3 gap-4">
            {formatKeys.map((key) => (
              <div key={key} className="rounded-lg border p-5 text-center">
                <h3 className="font-medium mb-2">{t(`format.${key}.title`)}</h3>
                <p className="text-sm text-muted-foreground">{t(`format.${key}.desc`)}</p>
              </div>
            ))}
          </div>
        </div>
